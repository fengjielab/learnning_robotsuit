...
class PickPlaceCans(SingleArmEnv):
    """
    ...
    def __init__(
        ...
        self.object_to_id = {"milk": 0, "bread": 1, "cereal": 2, "can": 3, "lemon": 4, "bottle": 5}
        self.object_id_to_sensors = {}  # Maps object id to sensor names for that object
        self.obj_names = ["Milk", "Bread", "Cereal", "Can", "Lemon", "Bottle"]
    ...
    def _get_placement_initializer(self):
        ...
        # print(self.model.mujoco_arena.table_full_size[0])
        # print(bin_x_half)
    ...
    def _setup_references(self):
        ...
        # print("len of objects_in_bins:  " + str(len(self.objects))) # 6
    ...
    def _reset_internal(self, obj_id_to_use=random.choice([0, 1, 2, 3, 4, 5])):
        ...
        if not self.deterministic_reset:
            ...
            for obj_pos, obj_quat, obj in object_placements.values():
                ...
                # elif "milk" in obj.name.lower(): # (can set as elif, can also merely use else)
                #     obj_pos = (0, 0, 0) # (can change value but not actual position)
                #     obj_quat = np.array([0.0, 0.0, 0.0, 0.0])
                #     self.sim.model.body_pos[self.obj_body_id[obj.name]] = obj_pos
                #     self.sim.model.body_quat[self.obj_body_id[obj.name]] = obj_quat
                # #     print(self.sim.model.body_pos[self.obj_body_id["Milk"]])
                # #     print(self.sim.model.body_quat[self.obj_body_id["Milk"]])
                else:
        ...
        # print("set milk")
        # obj_pos_fix = (-1, -1, 0)
        # obj_quat_fix = np.array([1.0, 0.0, 0.0, 0.0])
        # self.sim.model.body_pos[self.obj_body_id["Milk"]] = obj_pos_fix (can cahnge value but cannot change actual position)
        # self.sim.model.body_quat[self.obj_body_id["Milk"]] = obj_quat_fix
        # print(self.sim.model.body_pos[self.obj_body_id["Milk"]])  # cur: [0. 0. 0.]
        ...
        if self.single_object_mode in {1, 2}:
            ...
            # self.clear_objects(list(obj_names))  REVISION-4: do not move other objects out of the env scene
...
class PickPlaceCansSingle(PickPlaceCans):
    """
...
class PickPlaceCansMilk(PickPlaceCans):
    """
...
class PickPlaceCansBread(PickPlaceCans):
    """
...
class PickPlaceCansCereal(PickPlaceCans):
    """
...
class PickPlaceCansCan(PickPlaceCans):
    """
